...
def test_unix_http_connection() -> None:
    ...
    with mock.patch("socket.socket") as mock_socket_cls:
        # Mock AF_UNIX for Windows compatibility
        with mock.patch.object(socket, "AF_UNIX", create=True, new=1):
...
def test_unix_http_connection_options() -> None:
    ...
    conn = UnixHTTPConnection("/tmp/sock", socket_options=[(socket.SOL_SOCKET, socket.SO_KEEPALIVE, 1)], timeout=10)
    with mock.patch("socket.socket") as mock_socket_cls:
        with mock.patch.object(socket, "AF_UNIX", create=True, new=1):
            mock_sock = mock_socket_cls.return_value
            conn.connect()
