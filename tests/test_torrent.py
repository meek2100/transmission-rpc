...
def test_non_active():
    ...
    # verify that even if 0, it returns a valid datetime object (epoch 0 usually) or None depending on implementation.
    # checking the code, it seems it converts to datetime.
    assert torrent.activity_date, "activity_date should be a valid object even if timestamp is 0"
...
def test_attributes():
    ...
    torrent = transmission_rpc.Torrent(fields=data)
    assert torrent.done_date is None, "done_date should be None if 0"
...
def test_status():
    ...
    """
    assert Status("downloading").downloading, "Status('downloading') should identify as downloading"
    assert not Status("downloading").download_pending, "Status('downloading') should not identify as download_pending"
    assert Status("download pending").download_pending, "Status('download pending') should identify as download_pending"
    assert Status("download pending") in {"download pending", "o"}, "Status equality check failed"
